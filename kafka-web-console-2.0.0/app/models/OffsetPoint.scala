...
object OffsetPoint {
  ...
  def findByOffsetHistoryIdAndConsumerGroup(offsetHistoryId: Long, consumerGroup: String, from_time: Timestamp, to_time: Timestamp): Seq[OffsetPoint] = inTransaction {
    from(offsetPointsTable)(oP => where(oP.offsetHistoryId === offsetHistoryId and oP.consumerGroup === consumerGroup and oP.timestamp.between(from_time, to_time)) select (oP)).toList
  }
