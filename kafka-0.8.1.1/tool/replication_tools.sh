...
## Controlled Shutdown ##
./bin/kafka-run-class.sh kafka.admin.ShutdownBroker --zookeeper zk1dg.prod.mediav.com:2181/kafka --broker 0 --num.retries 3 --retry.interval.ms 60

## List Topic ##
./bin/kafka-topics.sh --zookeeper zk1dg.prod.mediav.com:2181/kafka --list
./bin/kafka-topics.sh --zookeeper zk1dg.prod.mediav.com:2181/kafka --describe --topic a.s.1

## Create Topic ##
./bin/kafka-topics.sh --zookeeper zk1dg.prod.mediav.com:2181/kafka --create --topic a.s.1 --partitions 30 --replication-factor 2
## Alter Retention Per Topic Partition
./bin/kafka-topics.sh --zookeeper zk1dg.prod.mediav.com:2181/kafka --create --topic a.s.1 --partitions 1 
        --replication-factor 1 --config retention.ms=86400000
./bin/kafka-topics.sh --zookeeper zk1dg.prod.mediav.com:2181/kafka --alter --topic a.s.1 
        --deleteConfig retention.ms

## Add Partition ##
# Partition Can only be Add, can't decrease !!!
./bin/kafka-topics.sh --zookeeper zk1dg.prod.mediav.com:2181/kafka --alter --topic a.s.1 --partition 40


## Delete Topic ##
# This script was bad. Only deleted in zookeeper, but log NOT deleted, so can not be trust !!!
./bin/kafka-run-class.sh kafka.admin.DeleteTopicCommand --zookeeper zk1dg.prod.mediav.com:2181/kafka --topic a.s.1
...
# for all:
./bin/kafka-preferred-replica-election.sh --zookeeper zk3dg.prod.mediav.com:2181/kafka
# for specified:
./bin/kafka-preferred-replica-election.sh --zookeeper zk3dg.prod.mediav.com:2181/kafka --path-to-json-file topicPartitionList.json
